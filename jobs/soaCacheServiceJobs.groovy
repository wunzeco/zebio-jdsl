...
job("${svc}-build") {
    ...
    steps {
        shell ( "cat >\$WORKSPACE/build.properties <<EOF\n" +
                ...
                "                       -G uk.co.o2.priority -A soa-cache-service -R priority-build-artifacts -N)\nEOF" )
        environmentVariables {
            propertiesFile('build.properties')
        }
        gradle 'clean startManagedMongoDb generateResourceDoc jacocoTestReport createCapsule distTarGz publish'
    }
    wrappers {
