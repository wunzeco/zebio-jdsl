...
job("${svc}-build") {
    ...
    steps {
        shell ( "cat >\$WORKSPACE/build.properties <<EOF\n" +
                ...
                "                       -G uk.co.o2.priority -A cms-service -R priority-build-artifacts -N)\nEOF" )
        environmentVariables {
            propertiesFile('build.properties')
        }
        gradle {
            tasks('clean startManagedMongoDb generateResourceDoc jacocoTestReport createCapsule distTarGz publish')
            switches('-Dorg.gradle.daemon=false -PgitCommit=\$GIT_COMMIT -PgitBranch=\$GIT_BRANCH -PgitURL=\$GIT_URL -PjenkinsBuild=\$BUILD_NUMBER')
        }
    }
    wrappers {
...
job("${svc}-build-image") {
    parameters {
        ...
        stringParam('APP_NAME',     defaultValue = 'cms-service', 
...
job("${svc}-deploy") {
    ...
    steps {
        ...
        shell(
            ...
            EXTRA_VARS=\"\$EXTRA_VARS kong_api_obj_upstream_url='http://sweb.\$DC_ENVIRONMENT.priority-infra.co.uk/${svc}-service/api'\"
