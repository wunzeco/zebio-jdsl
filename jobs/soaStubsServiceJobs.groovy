...
job("${svc}-build") {
    scm {
        git {
            remote {
                github("o2-priority/infra-soa-stubs", "ssh")
    ...
    steps {
        shell ( "cat >\$WORKSPACE/build.properties <<EOF\n" +
                ...
                "                       -G uk.co.o2.priority -A soa-stubs-service -R priority-build-artifacts -N)\nEOF" )
        environmentVariables {
            propertiesFile('build.properties')
        }
        gradle 'clean createDockerfile distTarGz publish'
    }
    wrappers {
...
job("${svc}-deploy") {
    ...
    steps {
        shell(
            """
            EXTRA_VARS=\"dockerize_app_name=\$APP_NAME dockerize_app_version=\$APP_VERSION\"
            EXTRA_VARS=\"\$EXTRA_VARS dockerize_app_conf_file=''\"
...
job("${svc}-test-functional") {
    ...
    steps {
        shell("echo 'Create a healthpage mapping on wiremock to hit'")
    }
}
